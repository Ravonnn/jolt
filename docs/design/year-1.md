...
**Definition of Done**
```
[x] A suite of Tiny programs runs and produces correct output via the interpreter
[x] `jolt run --interpret hello.jolt` works end to end
[x] `jolt test` discovers and runs [test] functions with pass/fail reporting
[x] The full pipeline (parse→type→custody→run) is exercised by CI on every PR
[x] Tutorial §1–§3 examples actually run (Tiny adaptations in `tests/tutorial/`)
[x] Known limitations (no generics/structs/concurrency yet) documented
```
